...
def cam2keypoints(data):
    ...
    valid_pairs, invalid_pairs = getValidPairs(
        outputs, w=w, h=h, detected_keypoints=new_keypoints
    )
...
def calculatePersonCrop(personDict, maxY, maxX):
    ...
    for person in personDict:
        ...
        # print (bottom)
        out.append((left, right, top, bottom))
    return out
...
def show(frame, keypoints_list, detected_keypoints, personwiseKeypoints):
    ...
    for idx in range(num_people):
        ...
        print(
            f"found person {idx+1} out of {num_bad_people+1} with {good_keypoints} keypoints"
        )  # min 7 keypoints

    if (
        keypoints_list is None
        or detected_keypoints is None
        or personwiseKeypoints is None
    ):
        return
    ...
    for crop in crops:
        ...
        out.append(frame[top:bottom, left:right])
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        detected_keypoints, keypoints_list, personwiseKeypoints = cam2keypoints(
            msgGrp["nn"]
        )
        ...
        # quit()
        crops = show(frameRaw, keypoints_list, detected_keypoints, personwiseKeypoints)
