...
nn = nm.createNN(pm.pipeline, pm.nodes, source=Previews.color.name, blobPath=Path(blob_path), fullFov=True)
pm.addNn(nn=nn)
...
with dai.Device(pm.pipeline, device_info) as device:
    pv = PreviewManager(display=[Previews.color.name], nnSource=Previews.color.name)
